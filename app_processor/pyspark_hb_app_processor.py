...
def find_top_n_locations(n=20):
    ...
    if RUN_WITH_VALIDATION:
        ...
        df_ge.expect_column_values_to_be_of_type("TOTAL_COUNT", "LongType")
        ...
        __validate_and_optionally_persist_expectation("task_b_find_top_n_locations", df_ge)
    return df
...
def find_timesheets_active_first_week():
    ...
    if RUN_WITH_VALIDATION:
        ...
        __validate_and_optionally_persist_expectation("task_c_find_timesheets_active_first_week", df_ge)
    return df
...
def find_events_grouped_by_location_device():
    ...
    if RUN_WITH_VALIDATION:
        ...
        df_ge.expect_column_values_to_be_of_type("TOTAL_EVENTS", "LongType")
        df_ge.expect_column_values_to_be_between("TOTAL_EVENTS", 0, None, True)
        __validate_and_optionally_persist_expectation("task_d_find_events_grouped_by_location_device", df_ge)
    return df
...
def init_dataframe():
    ...
    if RUN_WITH_VALIDATION:
        ...
        __validate_and_optionally_persist_expectation("task_a_init_dataframe", df_ge)
...
# Optionally persist expectations for each task. We can use these expectations to
# validate our dataframes against new data in the future
def __validate_and_optionally_persist_expectation(task_name, df_ge):
    if PERSIST_EXPECTATIONS:
    ...
    if len(failures) > 0:
        print("There are validation failures for task: ", task_name, ":\n", failures)
    else:
        print("Validation passed for task: ", task_name, "\n")
